...
AsyncHTTPRequest registration_post_request;
AsyncHTTPRequest device_input_post_request;
AsyncHTTPRequest device_inference_result_post_request;
AsyncHTTPRequest offloading_layer_get_request;
...
void postRegistration(){
  ...
  if (registration_post_request.readyState() == readyStateUnsent || registration_post_request.readyState() == readyStateDone)
  {
    String url = "http://" + String(HTTP_SRV) + ":" + String(HTTP_PORT) + "/api/registration";
    if (registration_post_request.open("POST", url.c_str()))
    {
      registration_post_request.setReqHeader("Content-Type", "application/json");
      ...
      registration_post_request.send(jsonMessage);
...
{
  if (device_input_post_request.readyState() == readyStateUnsent || device_input_post_request.readyState() == readyStateDone)
  {
    String url = "http://" + String(HTTP_SRV) + ":" + String(HTTP_PORT) + "/api/device_input";
    if (device_input_post_request.open("POST", url.c_str()))
    {
      // Set Content-Type header to application/octet-stream
      device_input_post_request.setReqHeader("Content-Type", "application/octet-stream");

      device_input_post_request.send((uint8_t*)fb->buf, fb->len);  // Send POST request with raw bytes body
    }
  }
}
...
void postDeviceInferenceResult(char* output_message, int size)
{
  if (device_inference_result_post_request.readyState() == readyStateUnsent || device_inference_result_post_request.readyState() == readyStateDone)
  {
    String url = "http://" + String(HTTP_SRV) + ":" + String(HTTP_PORT) + "/api/device_inference_result";
    if (device_inference_result_post_request.open("POST", url.c_str()))
    {
      // Set Content-Type header to application/octet-stream
      device_inference_result_post_request.setReqHeader("Content-Type", "application/octet-stream");

      device_inference_result_post_request.send((uint8_t*)output_message, size);  // Send POST request with raw bytes body
    }
  }
}
...
{
  if (offloading_layer_get_request.readyState() == readyStateUnsent || offloading_layer_get_request.readyState() == readyStateDone)
  {
    String url = "http://" + String(HTTP_SRV) + ":" + String(HTTP_PORT) + "/api/offloading_layer";
    if (offloading_layer_get_request.open("GET", url.c_str()))
    {
      offloading_layer_get_request.send();  // Send POST request with raw bytes body
    }
  }
}

void registrationCallback(void *optParm, AsyncHTTPRequest *request, int readyState)
{
  if (readyState == readyStateDone)
  {
    deviceRegistered = true;
    Serial.println("Response registration request:");
    Serial.println(request->responseText());
  }
}

void offloadingLayerCallback(void *optParm, AsyncHTTPRequest *request, int readyState)
{
  if (readyState == readyStateDone)
  {
    xTimerReset(deviceRegistrationTimer, 0);

    Serial.println("Response offloading layer request:");
...
void httpClientConfiguration() {
  registration_post_request.onReadyStateChange(registrationCallback);
  offloading_layer_get_request.onReadyStateChange(offloadingLayerCallback);
}
